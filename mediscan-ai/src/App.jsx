...
// ─── Storage Layer ─────────────────────────────────────────────────────────────
// Uses Artifact persistent storage API: window.storage
// Keys: "users" (shared registry), "sessions:{uid}" (per-user patient logs)

const DB = {
  // --- User registry (shared so usernames/emails are unique across sessions)
  async getUsers() {
    try { const r = await window.storage.get("medicheck:users", true); return r ? JSON.parse(r.value) : {}; }
    catch { return {}; }
  },
  async saveUsers(users) {
    try { await window.storage.set("medicheck:users", JSON.stringify(users), true); return true; }
    catch { return false; }
  },

  // --- Per-user patient session log (private to user)
  async getSessions(uid) {
    try { const r = await window.storage.get(`medicheck:sessions:${uid}`); return r ? JSON.parse(r.value) : []; }
    catch { return []; }
  },
  async saveSessions(uid, sessions) {
    try { await window.storage.set(`medicheck:sessions:${uid}`, JSON.stringify(sessions)); return true; }
    catch { return false; }
  },

  // --- Auth token (local session persistence — store logged-in uid)
  getToken() { try { return sessionStorage.getItem("medicheck:token"); } catch { return null; } },
  setToken(uid) { try { sessionStorage.setItem("medicheck:token", uid); } catch {} },
  clearToken() { try { sessionStorage.removeItem("medicheck:token"); } catch {} },
};

// Simple hash for password (client-side demo — not cryptographically strong)
function hashPw(pw) {
  let h = 0;
...
function LoginPage({ onLogin, goSignup }) {
  ...
  const submit = async () => {
    ...
    if(!users[key]) { setErr("No account found with this email. Please sign up."); setLoading(false); return; }
    if(users[key].passwordHash !== hashPw(password)) { setErr("Incorrect password. Please try again."); setLoading(false); return; }
    DB.setToken(users[key].uid);
    onLogin({ uid: users[key].uid, name: users[key].name, email: key });
    setLoading(false);
  };
...
function SignupPage({ onLogin, goLogin }) {
  ...
  const submit = async () => {
    ...
    const users = await DB.getUsers();
    const key   = email.trim().toLowerCase();
    if(users[key]) { setErr("An account with this email already exists. Please sign in."); setLoading(false); return; }
    const uid = `u_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
    users[key] = { uid, name: name.trim(), email: key, role, passwordHash: hashPw(password), createdAt: new Date().toISOString() };
    const ok = await DB.saveUsers(users);
    if(!ok) { setErr("Storage unavailable. Please try again."); setLoading(false); return; }
    DB.setToken(uid);
    onLogin({ uid, name: name.trim(), email: key, role });
    setLoading(false);
  };
...
export default function App() {
  ...
  // ── Restore session on mount (token from sessionStorage) ───────────────────
  useEffect(() => {
    const restore = async () => {
      const uid = DB.getToken();
      if(uid) {
        const users = await DB.getUsers();
        const found = Object.values(users).find(u => u.uid === uid);
        if(found) {
          setUser({ uid: found.uid, name: found.name, email: found.email, role: found.role });
          const saved = await DB.getSessions(found.uid);
          setSessions(saved);
        } else {
          DB.clearToken();
        }
  ...
  // ── Persist sessions to DB whenever they change ────────────────────────────
  useEffect(() => {
    if(user) DB.saveSessions(user.uid, sessions);
  }, [sessions, user?.uid]);
